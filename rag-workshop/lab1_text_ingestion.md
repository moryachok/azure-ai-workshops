...
- [Azure OpenAI Embedding](https://learn.microsoft.com/en-us/azure/search/cognitive-search-skill-azure-openai-embedding) - used to create vector embeddings out of textual data. Skill uses embedding models supported in Microsoft Foundry such as `text-embedding-3-small` and `text-embedding-3-large`.
- [Layout](https://learn.microsoft.com/en-us/azure/search/cognitive-search-skill-document-intelligence-layout) - uses [Azure Document Intelligence Layout Detection AI model](https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/prebuilt/layout?view=doc-intel-4.0.0&tabs=rest%2Csample-code) to detect document structure. Commonly let you apply smarter chunking strategy to capture content under same section even across multiple pages.

[Learn more](https://learn.microsoft.com/en-us/azure/search/cognitive-search-predefined-skills) about other supported skills.

## Upload data files into Azure Storage Account
...
**4.** Once the upload completes, verify that the files appear in the container under the `docx/` path and that each document is listed as a blob, confirming they are ready to be used as a data source for downstream indexing.
...
You will start with creating index. Index is similar to a database table. It stores data in a non-relational schema and enables full-text, vector and hybrid searches with millisecond performance. To learn more about indexes [visit this page](https://learn.microsoft.com/en-us/azure/search/search-what-is-an-index).

Index schema is semi-structured and can have nested fields. You must create the index before ingestion. Index design should support your expected query patterns, ensuring all required fields for search, filtering and retrieval are defined upfront. Once an index is created, existing fields cannot be modified or deleted. Only new fields can be added.

**1.** In the Azure portal, open Azure AI Search service. In the left menu, go to Search management -> Indexes -> Add index (JSON). ![Create Azure AI Search index](assets/create-index.png) 
...
Open `src/lab1/skillset.json` and review what it does.
...
- Uses `oneToMany` mode, meaning it will generate multiple output documents from a single input file (useful for multi-page documents or sections).
- Stores the result in a temporary `markdownDocument` field.
...
- You will see how `embedding` field will be mapped to `chunk_vector` field in `indexProjections` part of the skillset.
